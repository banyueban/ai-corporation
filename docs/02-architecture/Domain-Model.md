...
- 一个公司拥有多个任务；
- 一个员工可以加入多家公司，员工的模型和多项 Skill 配置仍只有一份；
- 一个已授权工作区可以加入多家公司；
...
### 1.2 PiEmployee 与 Skill

- 一名 `PiEmployee` 固定一个 Provider 和模型，可以分配一项或多项 Skill；
- Skill 分配使用有顺序的独立关系，不把多个名称拼进字符串；
- 任务开始时只加载该员工 Skill 的名称和用途；完整说明必须由员工按需启用；
- Skill 的文字、资源和 `allowed-tools` 都是不可信输入，不能改变工具权限；
- 原单 Skill 员工升级后保留原 Skill，成为列表中的第一项。
