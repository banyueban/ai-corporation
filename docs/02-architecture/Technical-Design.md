...
> **Pi 重启说明（2026-08-14）**：Electron、React、SQLite、工作区和桌面安全边界继续复用；新的员工运行基座改用 `@earendil-works/pi-ai` 与 `@earendil-works/pi-agent-core`。本文中的 Goal/Plan/DAG/固定编排流程属于旧路线，不再约束新的主流程。新的结构先由 M7-TU-01 做最小真实验证，验证通过后再逐步替换相应章节，避免先做一套没有跑过的“大设计”。
