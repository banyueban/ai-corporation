...
- UI 实现以 [UI/UX 文档中心](../07-ui/README.md)为设计基线；
- 状态由后端事实驱动，不猜测；
- 危险审批展示精确对象和副作用；
- 错误提供下一步；
- 长列表虚拟化；
...
- 不用动画掩盖未知进度；
- “AI 正在思考”不等于任务进度。
- Approval、Recovery、预算耗尽和验收失败必须使用统一交互，不允许页面自行弱化风险；
- 偏离信息架构、核心流程、状态含义或安全关键线框时，必须记录原因并评审；
- UI 变更按 [UI 专项验收标准](../07-ui/UI-Acceptance.md)提供组件、E2E、可访问性和跨平台证据。
...
- 小步提交；
- 每次实现只对应一个就绪任务单元；提交和 PR 必须标明任务单元 ID；
- 共享协议、迁移、状态机、CI 和项目状态文件按任务合同串行集成；
- PR 说明用户影响、风险、测试；
