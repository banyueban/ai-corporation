...
| 主要结果 | 用户从控制台进入长期轻量公司，管理可复用员工和常用工作区，在公司内发起并查看真实 Pi 任务；旧 Goal/Plan 数据保留但不提供界面入口。 |
...
- 用户在安装包验收中进一步确认：旧 Goal/Plan 数据不删除，但设置页也不保留旧版公司历史入口；
...
- 旧 Goal/Plan 数据原表保留但不提供界面入口，是用户在安装包验收中确认的产品边界。
...
- Renderer 不提供旧 Goal/Plan 的读取、写入按钮或隐藏调用入口。
...
- [x] 05 设置页不显示旧版公司历史入口；旧表、事件和关联记录保持不变；
- [x] 06 用户可以修改轻量公司名称；名称校验、重复提交、错误和重启恢复真实可见；
- [x] 07 同一员工可以加入两家公司，模型、Provider、Skill 和 Key 不复制；移出某公司不删除员工，也不影响其在另一公司或历史任务中的记录；
- [x] 08 同一 Workspace 可以加入两家公司；移出某公司不撤销全局授权、不删除用户文件，也不影响另一公司或历史任务；
- [x] 09 新任务只能选择当前公司的成员员工和当前可用、可写的常用工作区；伪造公司、员工或工作区关系在模型调用前固定拒绝；
- [x] 10 每项新任务持久化唯一 `companyId` 且不可换公司；Main/Storage 按公司查询，跨公司不能查看、继续、取消、验收或退回该任务；
- [x] 11 公司工作区显示本公司成员、常用工作区和各状态任务历史；历史任务在成员或工作区移出后仍可查看真实过程和结果；
...
- [x] 16 空库、仅员工、员工加任务、含旧 Corporation、含已完成/运行中 PiTask 的升级矩阵通过，`foreign_key_check` 无错误；
- [x] 17 开发态真实窗口完成“默认公司迁移 → 新建第二家公司 → 复用员工/工作区 → 发起并验收任务 → 跨公司隔离 → 设置无旧历史入口”的连续旅程；
...
- [x] 20 用户使用当前 Windows 安装包完成上述主流程并确认人工验收后，本任务和 Milestone 10 才能关闭。
...
- 旧数据测试只核对原表未被修改，界面中不存在旧 Corporation 入口，也不运行 Goal/Plan/Agent 服务；
