...
- [ ] 01 全新数据启动后显示清楚的空状态；用户只填写公司名称即可创建并进入公司，不出现 Goal、Plan、预算或组队步骤；
- [ ] 02 升级已有 Pi 数据时自动创建可改名的“我的公司”，现有员工、任务和任务使用过的工作区全部接入且数量、ID、内容、状态、事件和时间不变；
- [ ] 03 升级迁移不调用 Provider、模型、工具、命令或外部服务，重复启动和重复迁移不创建第二个默认公司或重复关系；
- [ ] 04 控制台只列出轻量公司；旧 Goal/Plan 公司不再出现在主流程，也不能从主流程继续、暂停、恢复或修改；
- [ ] 05 设置页“旧版历史”只读显示旧公司名称、状态、工作区展示路径和已有目标摘要；旧表、事件和关联记录保持不变；
...
- [ ] 12 从公司内创建员工时只创建一份全局员工配置并自动加入当前公司；已有员工可以直接加入，不要求重复填写模型和 Skill；
- [ ] 13 当前公司和最近任务在应用重启后恢复；已运行任务不因公司迁移、切换或重启被自动重放；
- [ ] 14 文本任务和编码任务继续完成读取、修改、命令、实时过程、失败/取消和人工验收，不重新引入旧 Goal/Plan；
- [ ] 15 Protocol strict Schema、IPC 授权、Repository、迁移、幂等、外键、关系移除和跨公司攻击测试通过；
...
- [ ] 18 公司控制台和公司工作区在 1024×700、1440×900 和 200% 缩放下无竖排、重叠、遮挡或关键操作不可达；
