...
- [x] 21 当前验收候选提交的 Windows x64 与 macOS Apple Silicon CI 均通过工程检查、真实窗口、应用构建、打包程序专项旅程和安装包上传；两平台证据不互相替代。
- [x] 22 用户使用当前 Windows 安装包完成环境计划、自动安装、脚本运行、过程和真实成果人工验收；22 项全部关闭且 P0/P1 为 0 后，M12-TU-02 与 Milestone 12 才能完成。
...
- 验收候选提交为 `66c924fa269be5e6ead878455e4aa885e7fd084c`；
- 本地 `pnpm check` 通过：协议 69、Provider 28、Storage 109、Desktop 194、Native Core 9、Workspace FS 11，并通过状态、任务合同、格式、类型、Clippy 和 Secret scan；
- 失败同例与完整开发态窗口回归通过：脚本环境同例 1/1 通过，完整主流程 9 条通过、5 条按当前入口或真实 Key 开关明确跳过；
- Windows 本地最终打包程序 6 条主流程通过，覆盖普通员工、编码员工、技能导入成功与失败、多 Skill 资源、独立环境安装、脚本运行和环境复用；
- GitHub Actions run `32869895594` 对同一候选提交通过：Windows x64 7 分 33 秒，macOS Apple Silicon 5 分 41 秒，两边均完成工程检查、真实窗口、应用构建、打包程序检查和安装包上传；
- 用户于 2026-08-26 使用当前 Windows 安装包确认技能导入、Python 独立环境计划、自动安装、脚本执行、过程可见、API Key 不显示明文、真实成果和第二次运行环境复用全部通过；
- 22 项验收断言全部关闭，P0/P1 为 0，没有新增已知 P2/P3；`DE-018`、`DE-019`、`DE-020` 继续作为明确后续增强，不属于当前缺陷；
- M12-TU-02 与 Milestone 12 完成；该结论不代表任意公开 Skill、更多脚本生态、附件、图片、视频、Office 专用能力或多员工协作已经交付。
