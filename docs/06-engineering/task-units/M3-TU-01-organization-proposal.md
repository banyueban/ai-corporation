...
- [ ] 01 只有当前 `APPROVED/VALID` Plan 显示并允许“开始组队”，批准动作本身不生成草案；
- [ ] 02 Renderer 不能伪造 Plan、Task、能力、工具、模板、角色、分配、Provider 或模型输入；
- [ ] 03 每次成功草案包含 1 个 Planner、按实际机器 Task 创建 0–3 个 Executor 和 1 个独立 Judge，总数不超过 5；存在非 `HUMAN_DECISION` Task 时至少有 1 个 Executor；
- [ ] 04 每个非 `HUMAN_DECISION` Task 恰好有一个 Executor 责任人；`HUMAN_DECISION` Task 恰好由用户负责；Judge 不与任何 Executor 共用身份；
- [ ] 05 相同业务输入重复计算得到相同模板、聚类、分工、职责分离和能力缺口；
- [ ] 06 内置模板具有稳定 ID 和版本，未知或不受信模板不能进入草案；
- [ ] 07 无法覆盖的能力形成含受影响 Task 和原因的结构化缺口，不虚构 Agent 或能力；
- [ ] 08 草案只含模型策略，不含精确 Provider/model 绑定，生成过程 Provider 调用次数为 0；
- [ ] 09 成功结果保存为递增的 `DRAFT` organization version，完整快照可审计；
- [ ] 10 相同命令重试返回原结果；命令内容冲突、当前 Plan 已变化或并发版本竞争被安全拒绝；
- [ ] 11 事务故障不留下半条草案或命令记录；Renderer 重载和应用重启后恢复当前草案；
- [ ] 12 生成成功、缺口或失败均不改变 Corporation `DRAFT`，不创建 Agent Instance/Run，不开始 Task；
- [ ] 13 Renderer/Main/Preload IPC allowlist、strict Schema 和固定安全错误一致；
- [ ] 14 中文 UI 清楚显示模板版本、角色、Task 分工、职责分离、能力缺口、模型策略以及“未激活、未执行”；
- [ ] 15 生成中禁止重复点击；失败不自动重试，保留已批准 Plan 并允许用户明确重试；
- [ ] 16 键盘、焦点、1024×700、1440×900 和 200% 缩放下可发起并查看完整草案；
- [ ] 17 Protocol、Storage、Desktop 单元/组件测试和迁移测试通过；
- [ ] 18 Windows 开发态真实 Electron 窗口完成批准 Plan 后主动组队、查看草案和重启恢复旅程；
- [ ] 19 当前提交 Windows/macOS CI 与最终包真实窗口检查通过，用户人工验收通过后方可关闭。
