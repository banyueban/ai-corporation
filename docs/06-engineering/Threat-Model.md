...
### T-16 恶意 Skill、资源与环境安装

场景：导入的 Skill 通过说明、`allowed-tools`、链接、超大文件、伪造资源路径或安装脚本提升权限、读取应用内部目录、覆盖工作区文件或诱导静默安装系统软件。

控制：

- Skill 只从用户明确导入后的应用自管副本使用；导入和分配本身不产生执行授权；
- 任务只允许启用当前员工已分配的 Skill，模型不能猜名称越权启用其他 Skill；
- Skill 的说明、`allowed-tools`、参考资料、资源和脚本全部按不可信外部内容处理，不能修改 Policy 或审批状态；
- 资源工具只接受受限相对路径，拒绝绝对路径、`..`、链接、目录、错误目录类型、二进制文本读取和超限文件；
- 复制资源时分别验证应用自管来源和当前任务 Workspace 目标，沿用并发保护与真实成果登记；
- 脚本仍经过现有任务命令授权；模型只能提交 Skill 名称、受限相对脚本路径、参数和结构化包名/版本，不能提交可执行路径、shell 命令、环境变量或工作目录；
- PEP 723、`requirements.txt`、`package.json` 和模型补充依赖都按不可信内容处理；拒绝 URL、Git、本地路径、额外管理器参数和命令字符；包安装脚本可能执行代码的风险在批准前明示；
- 自动安装默认进入 Skill 独立环境，项目环境只在 Skill 明确要求时使用；系统程序只允许受限 winget/Homebrew 计划并单独确认；
- uv/npm 版本与 uv 下载 SHA-256 随应用固定，Python 只安装到应用自管目录，不修改系统 PATH；管理器不可用时不尝试替代命令；
- 安装完成后必须复检并原子提交 `READY` 清单；中断、超时、复检失败或结果未知不重放、不运行脚本、不显示成功；
- 脚本和安装进程不继承 API Key、Key Vault 或应用认证秘密；它们以用户 OS 账户运行且没有 OS 级强沙箱，界面不得作相反宣传。

### T-17 恶意任务附件与文档输出

场景：用户选择的 Word/PDF 伪造扩展名、包含宏或恶意链接、选择后被替换，附件文字尝试诱导模型提升权限；生成文件越界、覆盖已有文件或以损坏内容冒充成功。

控制：

- 文件选择和拖放只由 Main/Preload 解析真实路径；Renderer、模型和 Skill 不接触原始绝对路径；
- 复制前后核对普通文件、链接状态、大小、扩展名、文件头和 SHA-256；附件进入按任务隔离的应用自管目录，原文件不修改；
- 只接受 `.docx`、`.pdf`、`.txt`、`.md`；拒绝 `.docm`、脚本、可执行文件、链接、目录、设备文件、伪扩展名和超限内容；
- 文档解析不执行宏、脚本、外部链接或嵌入对象；只把受限规范化文字片段发给模型，并把附件正文标记为不可信数据；
- 文档输出只允许当前 Workspace 中不存在的新 `.docx`/`.pdf` 相对路径，通过 Native Core 原子创建、哈希核对和成果登记；不允许覆盖附件、已有文件或越界路径；
- 损坏 Word、加密 PDF、无文字层扫描 PDF、解析超限和生成校验失败均返回准确错误，不显示成功；
- 依赖固定版本与许可证进入仓库检查，应用数据清理不能删除用户原文件或 Workspace 成果。
