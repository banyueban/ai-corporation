...
`0025_pi_task_collaboration.sql` 给 `pi_task` 增加 `task_mode`，只允许 `SINGLE` 或 `COLLABORATION`，默认 `SINGLE` 以保持旧任务语义；建立 `pi_task_assignment` 保存任务内分工、唯一员工、角色、顺序、状态、交接输出、受限失败和时间。协助员工必须由服务检查为当前公司成员，数据库外键只保证员工和任务存在。每次重新安排建立新行，不覆盖旧失败；任务删除级联删除分工。迁移不创建任务、不调用模型、不读写 Workspace，也不生成虚假交接。
