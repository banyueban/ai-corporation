...
flowchart TD
    ...
    COMPANY --> TASK["直接交代任务"]
    COMPANY --> COLLAB["开始公司协作"]
    TASK --> DELIVERY["过程、结果与人工验收"]
    COLLAB --> DELIVERY
    DASH --> SETTINGS["设置"]
```

公司工作区展示当前公司名称、成员员工、常用工作区和该公司任务历史。普通任务继续选择一名员工直接开始；公司协作从同一页面单独发起，用户选择工作区和最终负责人，最终负责人按任务与成员 Skill 选择协助员工。任务必须在公司上下文内发起；员工和工作区可被多家公司复用，但任务只能显示在所属公司。
