...
| 状态 | 成果区行为 | 可用操作 |
| --- | --- | --- |
| RUNNING | 显示已登记的阶段成果并标注任务仍在进行，不提前显示交付成功 | 查看阶段成果、展开完整过程 |
| WAITING_ACCEPTANCE | 显示员工总结、真实文件、差异和最终检查，标注“等待你验收” | 预览、打开安全文件、查看所在位置、验收或退回 |
| COMPLETED | 显示已验收成果和真实文件当前状态 | 预览、打开安全文件、查看所在位置 |
| FAILED / CANCELLED / INTERRUPTED | 已登记文件显示为“未完成成果”，同时显示准确原因 | 预览已有文件、查看所在位置、重新执行 |
| No registered file | 显示“没有已登记文件”；旧总结和完整过程继续可见 | 展开完整过程 |
...
| 状态 | UI | 可用操作 |
| --- | --- | --- |
| Import preview | 文件夹有效时，变化确认卡自动进入当前视口并获得焦点 | 确认导入、取消 |
| Import rejected | 导入按钮附近显示准确原因并获得焦点；技能列表保持原样 | 重新选择文件夹 |
| Import completed | 导入成功提示、技能卡和新员工技能选项立即出现 | 创建或编辑员工 |
...
| 状态 | 主区域 | 可用操作 |
| --- | --- | --- |
| Ready to start | 显示最终负责人、可写公司工作区、任务内容和附件；少于两名成员时贴近入口说明 | 开始公司协作 |
| RUNNING / selecting helpers | 显示最终负责人正在理解任务；分工产生后立即追加，不显示虚假固定团队 | 查看过程、停止任务 |
| RUNNING / helper | 每条分工显示员工、模型、Skill、说明、状态和交接；失败卡保留准确原因 | 查看员工过程、停止任务 |
| RUNNING / finalizing | 显示最终负责人正在使用哪些已完成交接；失败交接不会消失 | 查看过程、停止任务 |
| WAITING_USER | 置顶显示不能继续的原因、失败分工、已有结果和三种后果 | 使用现有结果继续、重新安排失败工作、停止任务 |
| WAITING_ACCEPTANCE | 显示最终负责人总结、全部分工与交接、最终真实文件和检查 | 验收通过、退回最终负责人修改 |
| COMPLETED | 显示用户已验收的整体结果；单个分工状态保持可追溯 | 查看成果和完整过程 |
| FAILED | 显示最终负责人或运行失败的准确原因，已完成交接和文件标为未完成成果 | 重新执行新任务 |
| CANCELLED / INTERRUPTED | 未结束分工显示取消或中断；已成功交接和文件保留，不声称整体完成 | 查看已有结果、重新执行新任务 |

### 8.4 任务附件与文档

| 状态 | UI | 可用操作 |
| --- | --- | --- |
| No attachment | 任务输入区显示支持 Word、PDF、文本和 Markdown | 选择文件、拖入文件 |
| Staging | 显示正在复制，不允许开始任务 | 等待、取消选择 |
| Ready | 每个附件显示名称、类型、大小，不显示本机路径 | 移除、继续添加、开始任务 |
| Rejected | 在附件区显示准确文件名和原因，其他有效附件不丢失 | 移除问题文件、重新选择 |
| Task running | 附件列表固定，只读显示；过程区显示分段读取 | 查看过程、停止任务 |
| Scanned PDF | 显示“这个 PDF 没有可读取文字，当前版本暂不支持扫描件文字识别” | 更换文件、结束或退回 |
| Document created | 成果区显示新 Word/PDF、真实大小和当前状态 | 查看内容、打开文件、查看所在位置 |
