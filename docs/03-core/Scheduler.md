...
M3-TU-03 的首个启动切片不提前实现上述完整评分。用户明确开始执行时，只对当前 Plan 中依赖为空的任务按 `user_priority` 降序排序；同级时 `HUMAN_DECISION` 优先，再按任务持久化创建时间和 Task ID 稳定排序。该临时规则只用于一次性选择首任务，不形成未来完整 Scheduler 的评分权重或并发策略。
