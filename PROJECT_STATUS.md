...
| --- | --- |
| 当前产品版本 | Pi 重启版真实工作阶段 |
| 当前阶段 | Milestone 14 已形成开发态验收候选，正在验证最终 Windows 安装包 |
| 当前 Milestone | Milestone 14：任务附件与文档处理（进行中） |
| 当前任务单元 | M14-TU-01（进行中，尚未完成） |
| 总体状态 | Milestone 7–13 全部完成；M14-TU-01 已实现附件副本、独立标准文档 Skill、通用文档工具和 Word/PDF 真实闭环，本地完整检查与开发态窗口回归通过；最终安装包、双平台 CI 和用户人工验收尚未完成 |
| 最近更新 | 2026-09-03 |
| 下一检查点 | 生成 Windows 安装包并运行同一条真实文档旅程，修复打包差异后提交并等待双平台 CI |
...
M14-TU-01 正在验收：任务已可选择或拖入 Word、PDF、文本和 Markdown，软件保存按任务隔离的附件副本；内置独立标准文档 Skill 和其他已启用标准 Skill 可复用通用工具，分段读取附件并生成新的 Word/PDF。本地完整检查 205 项桌面测试、110 项存储测试、71 项协议测试、28 项 Provider 测试及 23 项 Rust 测试全部通过；开发态真实窗口 11 条通过、5 条按旧入口或真实 Key 开关跳过。扫描 PDF OCR、复杂排版、PPT、表格、图片、视频和多员工不在本任务范围；最终安装包、双平台 CI 和用户人工验收仍待完成。
...
当前没有活跃阻塞。用户已为 M14-TU-01 确认 `1A + 2B + 3A + 4A + 5A`，并同意按说明实现，采用 `6A + 7B`：附件保存应用副本；首版支持 Word、文本、Markdown 和普通 PDF；输出永远是新文件；使用标准 Skill 与通用工具；保留常用结构；扫描 PDF OCR 后续增加；由 AI Corporation 独立编写标准文档 Skill，不复制 Anthropic 禁止再分发的 `docx`/`pdf` 内容。
