...
| --- | --- |
| 当前产品版本 | Pi 重启版真实工作阶段 |
| 当前阶段 | Milestone 14 设计与任务合同已就绪，准备实施任务附件与文档处理 |
| 当前 Milestone | Milestone 14：任务附件与文档处理（进行中） |
| 当前任务单元 | M14-TU-01（就绪） |
| 总体状态 | Milestone 7–13 全部完成；M14-TU-01 已按用户确认的附件副本、Word/PDF、永不覆盖、标准 Skill、常用结构和普通 PDF 范围建立合同，尚未产生功能实现或验收证据 |
| 最近更新 | 2026-09-02 |
| 下一检查点 | 完成附件协议、SQLite 迁移和应用自管副本，再接入文档读取与生成工具 |
...
M14-TU-01 已就绪：任务允许选择或拖入 Word、PDF、文本和 Markdown，软件保存按任务隔离的附件副本；内置独立标准文档 Skill 通过通用工具分段读取并生成新的 Word/PDF。扫描 PDF OCR、复杂排版、PPT、表格、图片、视频和多员工不在本任务范围，尚未开始实现。
...
- [ ] 任务附件选择、拖放、应用副本、数据迁移和恢复；
- [ ] `document-processing` 标准 Skill 与通用读取/生成工具；
- [ ] Word/普通 PDF 真实读取、新 Word/PDF 生成和成果展示；
- [ ] 本地完整检查、开发态与最终打包程序验收；
- [ ] Windows/macOS CI；
- [ ] 用户 Windows 安装包人工验收。
