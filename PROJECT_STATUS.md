...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | Milestone 15 轻量多员工协作正在确认关键产品行为，尚未开始实施                                                                                                                                                                                          |
| 当前 Milestone | Milestone 15：轻量多员工协作（规划中）                                                                                                                                                                                                                 |
| 当前任务单元   | M15-TU-01（未开始，关键产品决策未确认）                                                                                                                                                                                                                 |
| 总体状态       | Milestone 7–14 全部完成；用户已选择下一方向为轻量多员工协作，并要求把正式版 UI 设计与重做加入后续方向；M15 尚无就绪合同，不得开始实现                                                                                                                    |
| 最近更新       | 2026-09-07                                                                                                                                                                                                                                              |
| 下一检查点     | 用户确认 M15 的组队方式、协作写入、验收责任和首条真实旅程；随后更新权威设计并建立 M15-TU-01                                                                                                                                                            |
...
Milestone 15 已确定方向但尚未开始实现：交付轻量多员工协作，不恢复旧 Goal/Plan/DAG 和固定三角色流程。组队方式、员工怎样共享结果与写入 Workspace、谁负责验收以及首条真实验收任务会改变数据、运行和 UI 设计，必须由用户确认后再建立任务合同。正式版 UI 设计与重做已登记为独立后续方向 `DE-022`，不会混在 M15 中用零散美化冒充完成。
...
- 普通 PDF 可读取和生成，Word/PDF 共同保留标题、段落、粗体、斜体、删除线、行内代码、引用、分隔线、安全链接、两级列表和带行内格式的表格，不显示对应 Markdown 标记；
...
当前需要用户确认 M15 的关键产品行为，确认前不得修改多员工协议、数据或实现。用户已明确选择轻量多员工协作为下一方向，并要求正式版 UI 设计与重做进入后续选项；后者已登记为 `DE-022`。

M14-TU-01 的既有决策保持不变：附件保存应用副本；首版支持 Word、文本、Markdown 和普通 PDF；输出永远是新文件；使用标准 Skill 与通用工具；扫描 PDF OCR 后续增加；由 AI Corporation 独立编写标准文档 Skill，不复制 Anthropic 禁止再分发的 `docx`/`pdf` 内容；Word/PDF 共同支持已验收的常用格式，图片、复杂公式、脚注和任意 HTML 留待以后。
