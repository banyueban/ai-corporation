...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | Milestone 14 已完成，任务附件与 Word/PDF 文档处理闭环通过用户人工验收                                                                                                                                                                                   |
| 当前 Milestone | Milestone 14：任务附件与文档处理（完成）                                                                                                                                                                                                                |
| 当前任务单元   | M14-TU-01（完成）                                                                                                                                                                                                                                      |
| 总体状态       | Milestone 7–14 全部完成；M14-TU-01 的附件保存、文档读取、Word/PDF 生成、常用格式、成果展示、本地检查、Windows 最终包、Windows/macOS CI 和用户人工验收均已通过，P0/P1 为 0                                             |
| 最近更新       | 2026-09-07                                                                                                                                                                                                                                              |
| 下一检查点     | 与用户确认下一个产品方向并建立新的就绪任务合同；未确认前不自行扩大范围                                                                                                                                                                                 |
...
当前没有活跃阻塞。用户已为 M14-TU-01 确认 `1A + 2B + 3A + 4A + 5A`，并同意按说明实现，采用 `6A + 7B`；macOS CI 暴露系统字体无法进入生成 PDF 后，用户进一步选择 A 方案，批准随软件提供 Noto Sans SC。附件保存应用副本；首版支持 Word、文本、Markdown 和普通 PDF；输出永远是新文件；使用标准 Skill 与通用工具；扫描 PDF OCR 后续增加；由 AI Corporation 独立编写标准文档 Skill，不复制 Anthropic 禁止再分发的 `docx`/`pdf` 内容。重新人工验收发现 Markdown 格式未正确转换后，用户再次选择 A：Word/PDF 共同支持常用格式，图片、复杂公式、脚注和任意 HTML 留待以后。
