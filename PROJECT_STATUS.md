...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | Milestone 14 Word/PDF 常用格式修复已通过本地、Windows 最终包和 Windows/macOS CI，等待用户人工验收                                                                                                                                                      |
| 当前 Milestone | Milestone 14：任务附件与文档处理（进行中）                                                                                                                                                                                                              |
| 当前任务单元   | M14-TU-01（进行中，尚未完成）                                                                                                                                                                                                                           |
| 总体状态       | Milestone 7–13 全部完成；M14-TU-01 已按用户选择的 A 方案补齐 Word/PDF 常用格式转换，本地完整检查、开发态真实窗口、Windows 最终包和当前提交的 Windows/macOS CI 全部通过；只剩用户使用新安装包人工验收，任务仍未完成                      |
| 最近更新       | 2026-09-07                                                                                                                                                                                                                                              |
| 下一检查点     | 用户安装 `release/M14-9a5a02c/AI Corporation Desktop Setup 0.1.0.exe`，用真实内容生成 Word/PDF 并核对常用格式                                                                                                                                          |
...
M14-TU-01 仍在进行中：长文档 PDF 地址过长、成果文件名误当附件 ID 和乱留探测脚本问题已经修复；重新人工验收发现的 Markdown 格式问题也已按用户选择的 A 方案修复，Word/PDF 现在共同支持标题、粗体、斜体、删除线、行内代码、引用、分隔线、安全链接、两级列表和带行内格式的表格。本地完整检查、开发态 11 条、Windows 最终包 8 条以及提交 `9a5a02c` 的 Windows/macOS CI 全部通过；用户人工验收尚未通过，任务不能标记完成。
...
- 当前本地验收包位于 `release/M14-9a5a02c/AI Corporation Desktop Setup 0.1.0.exe`，大小 152753697 字节，SHA-256 为 `4E089F68B1C36A23DEBD69AECFD7F6EA6447F66F1AE29600C6BB930799F8CA8F`；GitHub CLI 下载远端 Windows artifact 两次都长时间无字节落盘，因此没有把未完成下载冒充 CI 本地副本。该本地包来自同一源码并已通过本机最终包 8 条旅程，仍需用户人工验收。
