...
| --- | --- |
| 当前产品版本 | Pi 重启版真实工作阶段 |
| 当前阶段 | Milestone 12 首个任务已完成，第二个任务尚未开始 |
| 当前 Milestone | Milestone 12：标准 Agent Skills 运行底座 |
| 当前任务单元 | M12-TU-01（完成） |
| 总体状态 | Milestone 7–11 和 M12-TU-01 已完成；M12-TU-02 的脚本运行、环境检查和隔离自动安装尚未开始，因此 Milestone 12 仍在进行中 |
| 最近更新 | 2026-08-24 |
| 下一检查点 | 建立并审查 M12-TU-02 任务合同，按已确认的 `5C + 6A` 实现独立环境管理工具、脚本运行和项目或 Skill 独立环境自动安装 |
...
M12-TU-01 已完成：一名员工可选择多项 Skill，旧员工会保留原 Skill；任务开始时模型只看到名称和用途，再按任务启用完整说明、读取参考资料或复制资源成真实成果。Windows/macOS 自动检查和用户 Windows 安装包人工验收均已通过。脚本运行和环境自动安装仍属于 M12-TU-02，没有混入已完成任务。
...
M12-TU-01 已按以下边界完成标准 Skill 的第一段独立闭环：
...
当前没有外部阻塞。M12-TU-01 已关闭；下一步只能先建立并审查 M12-TU-02 合同，再实施脚本和环境能力。用户已确认 M12 路线：一名员工拥有多项 Skill，员工按任务自动选择；使用专门工具启用 Skill、列出/读取参考资料、复制资源和运行脚本；缺少环境时由独立环境管理工具提供自动安装，优先安装到项目或 Skill 独立环境，系统级安装必须再次确认。未来环境员工复用同一工具，真实公开 Skill 另做端到端验收。
