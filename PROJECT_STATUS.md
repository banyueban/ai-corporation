...
| 总体状态 | Milestone 7–9 已完成；M10-TU-01 已完成本地实现、工程检查、开发态、Windows 打包程序和 GitHub Windows/macOS CI 自动验收，用户安装包人工验收尚未完成 |
| 最近更新 | 2026-08-21 |
| 下一检查点 | 用户使用当前 Windows 安装包人工验收；通过后再关闭 M10-TU-01 和 Milestone 10 |
...
- [x] 主控制台只保留轻量公司，旧公司在设置中只读查看；
- [x] 本地工程检查、开发态窗口和 Windows 打包程序自动验收通过；
- [x] 当前提交的 Windows/macOS CI 通过；
...
- 旧 Goal/Plan 公司退出主流程，只在设置中只读保留。

本任务不做公司删除、归档、导入导出、多员工自动协作或新增图片、视频、文档、PPT 等工具能力。

## 5. 活跃阻塞与已确认决策

当前没有活跃阻塞。用户已确认 M10-TU-01 采用 `1A + 2A + 3A`：新建独立轻量公司；升级时自动创建“我的公司”接住已有 Pi 数据；员工和工作区可跨公司复用，每项任务只属于一家公司；旧 Goal/Plan 公司在设置中只读保留。
...
- 本地 `pnpm check` 通过：协议 61、Provider 28、存储 106、桌面 156、Native Core 7、Workspace FS 9，并通过格式、类型、Clippy 和 Secret scan；
- 开发态完整窗口验收为当前主流程 5 条通过、5 条明确跳过；其中 4 条是已退出主入口的旧 Goal/Plan 写流程，1 条是需显式启用本地 Key 的真实 Provider 专项；轻量公司文本与编码旅程 2 条全部通过；
- Windows 最终包已使用最终提交重新生成，打包程序的轻量公司文本与编码旅程 2 条全部通过；当前安装包大小 106168284 字节，SHA-256 为 `2F36F102952C81751F3049F7AEB0D419F5AC112E299F6D48AD4DDA879DEEE9BE`；
- 提交 `cc8b36a0da66d83f3df1c65adab26cc69cf7ae22` 的 GitHub Actions run `32490490631` 已通过：macOS Apple Silicon（4 分 9 秒）和 Windows x64（7 分 20 秒）均完成工程检查、真实窗口、安装包构建、打包程序窗口检查和安装包上传；
- 当前提交尚未取得用户安装包人工验收，因此 M10-TU-01 与 Milestone 10 保持部分完成。
...
Milestone 10 在实施前完成了旧公司与 Pi 路线关系核对：旧 `corporation` 是一次 Goal/Plan 执行实例，而 `pi_employee` 和 `pi_task` 原本没有公司归属。直接复用旧表会重新引入旧状态机并混淆历史，因此新旧数据必须分离；迁移、成员复用、任务唯一归属和旧历史只读边界已写入 M10-TU-01。
