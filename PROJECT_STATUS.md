...
| 总体状态 | Milestone 7–13 全部完成；M14-TU-01 已实现附件副本、独立标准文档 Skill、通用文档工具和 Word/PDF 真实闭环；Windows 本地检查与打包程序已通过，macOS CI 发现系统字体未写入 PDF，正在按用户批准的 A 方案内置 Noto Sans SC；修复后的双平台 CI 和用户人工验收尚未完成 |
| 最近更新 | 2026-09-03 |
...
M14-TU-01 正在验收：任务已可选择或拖入 Word、PDF、文本和 Markdown，软件保存按任务隔离的附件副本；内置独立标准文档 Skill 和其他已启用标准 Skill 可复用通用工具，分段读取附件并生成新的 Word/PDF。Windows 本地完整检查、开发态窗口和最终打包程序此前已通过；macOS CI 保存下来的失败 PDF 只有表格线、没有任何文字，已经确认是系统字体未嵌入，而不是模型或 PDF 读取误报。用户选择 A 方案，当前正在让安装包内置 Noto Sans SC，并按文档实际字符加载字体片段。扫描 PDF OCR、复杂排版、PPT、表格、图片、视频和多员工不在本任务范围；修复后的双平台 CI 和用户人工验收仍待完成。
