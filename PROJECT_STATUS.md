...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | Milestone 14 真实长文档修复已通过本地、Windows/macOS CI 和最终包验证，等待用户重新人工验收                                                                                                                                                              |
| 当前 Milestone | Milestone 14：任务附件与文档处理（进行中）                                                                                                                                                                                                              |
| 当前任务单元   | M14-TU-01（进行中，尚未完成）                                                                                                                                                                                                                           |
| 总体状态       | Milestone 7–13 全部完成；M14-TU-01 首轮人工验收暴露的长文档 PDF 地址过长、成果文件名误当附件 ID 和乱留探测脚本问题已修复；本地完整检查、Windows 最终包及当前提交的 Windows/macOS CI 全部通过；只剩用户使用当前 Windows 安装包重新人工验收，任务仍未完成 |
| 最近更新       | 2026-09-07                                                                                                                                                                                                                                              |
| 下一检查点     | 用户安装 `release/M14-1a1031a/AI Corporation Desktop Setup 0.1.0.exe`，用真实附件重新生成 Word/PDF 并核对任务状态、过程与成果                                                                                                                           |
...
M14-TU-01 正在等待用户重新验收：首轮真实附件任务最终已生成并核对 Word/PDF，但前面的长 PDF 生成曾因多个内置字体片段让 `data:` 地址超过 Chromium 上限而失败；模型随后又把成果文件名误填为附件 ID，最后两次工具失败使任务如实标记失败。修复改用打印后立即清理的应用私有临时 HTML，明确区分原始附件与生成成果，并禁止员工在内置工具失败后擅自留下环境探测脚本；覆盖全部内置字体片段的开发态和 Windows 最终包文档旅程、完整 `pnpm check` 以及 GitHub Actions `34108922528` 的 Windows/macOS 开发态 11 条与最终包 8 条旅程均通过。CI Windows 安装包位于 `release/M14-1a1031a/AI Corporation Desktop Setup 0.1.0.exe`，大小 152753954 字节，SHA-256 为 `98E7F52B6EA65FE1A3E1E82EF91105FC342C5F36BC20B88216932B46B879E559`；用户重新人工验收前，任务不能标记完成。
