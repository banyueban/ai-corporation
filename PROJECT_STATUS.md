...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | Milestone 14 Word/PDF 常用格式修复已通过本地和 Windows 最终包验证，等待同一提交的 Windows/macOS CI                                                                                                                                                     |
| 当前 Milestone | Milestone 14：任务附件与文档处理（进行中）                                                                                                                                                                                                              |
| 当前任务单元   | M14-TU-01（进行中，尚未完成）                                                                                                                                                                                                                           |
| 总体状态       | Milestone 7–13 全部完成；M14-TU-01 已按用户选择的 A 方案补齐 Word/PDF 常用格式转换，本地完整检查、开发态真实窗口和 Windows 最终包 8 条旅程通过；尚未提交当前源码的双平台 CI，用户也尚未使用新包复验，任务仍未完成                        |
| 最近更新       | 2026-09-07                                                                                                                                                                                                                                              |
| 下一检查点     | 提交并推送当前候选，取得同一提交的 Windows/macOS CI 与安装包 artifact                                                                                                                                                                                  |
...
M14-TU-01 仍在进行中：长文档 PDF 地址过长、成果文件名误当附件 ID 和乱留探测脚本问题已经修复；重新人工验收发现的 Markdown 格式问题也已按用户选择的 A 方案修复，Word/PDF 现在共同支持标题、粗体、斜体、删除线、行内代码、引用、分隔线、安全链接、两级列表和带行内格式的表格。当前本地完整检查、开发态 11 条和 Windows 最终包 8 条真实窗口旅程通过；新源码尚无双平台 CI 和用户人工验收，任务不能标记完成。
...
- Windows 最终打包程序 8 条真实窗口旅程全部通过；当前本地安装包位于 `release/AI Corporation Desktop Setup 0.1.0.exe`，大小 152753697 字节，SHA-256 为 `4E089F68B1C36A23DEBD69AECFD7F6EA6447F66F1AE29600C6BB930799F8CA8F`；该包尚未取得当前提交的双平台 CI，也尚未由用户人工验收。
