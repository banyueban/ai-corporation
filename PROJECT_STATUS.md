...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | M15-TU-01 已完成本地实现、完整工程检查、开发态、Windows 最终程序和双平台 CI 自动验收，等待用户人工验收                                                                                                                                                    |
| 当前 Milestone | Milestone 15：轻量多员工协作（实施中）                                                                                                                                                                                                                 |
| 当前任务单元   | M15-TU-01（进行中，等待用户人工验收）                                                                                                                                                                                                                   |
| 总体状态       | Milestone 7–14 全部完成；M15-TU-01 本地完整检查、失败/继续/取消/退回/重启窗口证据、Windows 最终程序和双平台 CI 已通过，但用户人工验收尚未关闭，不标记完成                                                                                                  |
| 最近更新       | 2026-09-08                                                                                                                                                                                                                                              |
| 下一检查点     | 请用户安装当前 Windows 包，人工验收公司协作主流程和失败/停止/退回边界                                                                                                                                                                                     |
...
Milestone 15 已进入首个任务单元实施：多名员工共享同一个 Workspace，一个文件只有一名负责人；编码任务只由一名编码员工修改代码和运行项目命令，其他员工通过独立结果或检查意见协助；不创建 worktree、员工分支或自动合并。公司协作从公司工作区发起，用户选择最终负责人和 Workspace，最终负责人根据当前公司员工的 Skill 自行安排协助员工且不二次确认。协助员工失败不自动重试；负责人能继续就使用已有结果继续，不能继续就询问用户；取消会停止仍在运行的员工并保留未完成成果。M15-TU-01 先验收文档协作，M15-TU-02 再验收编码协作，两项都通过后才能关闭 Milestone。正式版 UI 设计与重做已安排为 Milestone 15 后的独立方向 `DE-022`。
...
- M15-TU-01 已完成本地实现、失败/继续/停止/退回/重启窗口验收、Windows 最终程序和双平台 CI 自动验收，等待用户人工验收；M15-TU-02 未开始，没有任何多员工功能被标记为完成。

## 4. 当前任务边界

当前允许按已就绪合同实施 M15-TU-01。实施边界是：
...
合同就绪不能被当成功能实现、开发态验收、最终包验收或 Milestone 完成。

## 5. 活跃阻塞与已确认决策

当前没有阻止 M15-TU-01 实施的产品决定。用户已选择 `1A + 2A`：从公司工作区发起，由用户选择最终负责人和 Workspace；负责人按 Skill 安排协助员工且不二次确认；失败不自动重试，无法继续时询问用户；取消停止仍在运行的员工并保留未完成成果，不自动恢复或重放。
...
- 当前仍未完成 M15-TU-01：用户 Windows 安装包人工验收尚未通过；“重新安排失败工作”和“在等待决定时直接停止”已有界面、服务与自动测试证据，但本轮连续窗口只实际点击了“使用已有结果继续”，不把未点击的分支写成完整窗口通过。

## 7. 阶段复盘与下一步

当前下一步是由用户安装当前 Windows 包，人工验收公司协作主流程和失败处理。通过前保持“进行中”；通过后关闭 M15-TU-01 并启动 M15-TU-02，Milestone 15 完成后进入正式版 UI 设计与重做。
