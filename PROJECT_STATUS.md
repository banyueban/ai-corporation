...
按任务单元规范完成 Milestone 7 阶段复盘，检查目标与实际交付、布局返工等过程问题、有效控制和简化清单，再据此定义下一 Milestone 与首个可独立验收的任务合同。新合同就绪前不得开始实现。
