...
| 当前任务单元 | M9-TU-01（进行中） |
| 总体状态 | Milestone 8 已完成；M9-TU-01 本机实现与最终包自动验收通过，等待当前提交跨平台 CI 和用户安装包人工验收 |
| 最近更新 | 2026-08-21 |
| 下一检查点 | 提交并通过当前代码的 Windows/macOS CI，再交付安装包供用户人工验收 |
...
- [ ] 当前提交 Windows/macOS CI 与安装包 artifact 通过；
...
当前没有仓库或实现阻塞。任务关闭还需要当前提交通过 Windows/macOS CI，并由用户使用当前 Windows 安装包完成人工验收。
...
- 当前 Windows 安装包位于 `release/AI Corporation Desktop Setup 0.1.0.exe`，大小 106164185 字节，SHA-256 为 `D6939485FA1FAAEE8FC46D36D71462036E85A878D3631ABC7CE1B51D4B707A91`；当前提交 CI 和用户人工验收尚未完成，因此 M9-TU-01 与 Milestone 9 保持“进行中”。
...
M9-TU-01 的本机功能和自动验收已经达到候选交付状态；其中命令和代码检查认领 `DE-014` 的一部分，没有 OS 级强隔离的阶段性让步登记为 `DE-015`。当前只等待跨平台 CI 和用户安装包人工验收，图片、视频、文档、PPT、附件和多员工仍未交付。
