...
| --- | --- |
| 当前产品版本 | Pi 重启版真实工作阶段 |
| 当前阶段 | Milestone 11 人工验收发现的两个故障已修复，本地与 Windows/macOS CI 已通过，等待用户复验 |
| 当前 Milestone | Milestone 11：可信交付成果 |
| 当前任务单元 | M11-TU-01（进行中） |
| 总体状态 | Milestone 7–10 已完成；M11-TU-01 的老员工 Provider 版本误判和“查看所在位置”不打开文件夹已修复，完整工程检查、开发态、Windows 打包程序和提交 `257d6ec` 的 Windows/macOS CI 均通过；必须由用户使用新安装包复验后才能完成 |
| 最近更新 | 2026-08-22 |
| 下一检查点 | 用户使用新 Windows 安装包复验老员工任务和“查看所在位置” |
...
- [ ] 文本/代码预览、文件外部变化、系统打开和所在位置已完成自动验证，等待用户对文件夹打开修复复验；
- [ ] Windows 开发版、最终打包版、Windows/macOS CI 和用户人工验收通过。

## 4. 当前任务边界

M11-TU-01 把真实交付成果接到 Pi 主流程：

- 受控写入成功后自动登记成果；命令生成文件由员工调用登记工具，软件核对真实文件后登记；
- 结果页先展示员工总结、真实文件和检查结果，完整模型与工具过程继续保留但默认折叠；
- 文本和代码成果支持软件内预览、系统打开和查看所在位置；路径、公司、任务和 Workspace 归属必须由可信边界核对；
- 失败、取消和中断任务保留已登记文件，但明确标记为未完成成果；旧任务不猜测或回填结构化成果。

本任务不做标准 Agent Skills 完整运行底座、全工作区变更扫描、复杂 Artifact 版本树、附件、多员工或图片、视频、Office 等专用工具。

## 5. 活跃阻塞与已确认决策

当前没有外部阻塞。用户已确认下一路线按“统一交付成果区 → 标准 Agent Skills 运行底座 → 真实标准 Skill 端到端验收 → 附件与文档能力”推进；M11-TU-01 采用“受控写入自动登记，命令产物经 `workspace.register_deliverable` 核实后登记”的方案，不扫描整个工作区，也不把模型文字当成文件证据。
...
Milestone 11 先解决“员工做完以后，用户到底拿到了什么”。首次人工验收发现两项真实故障：Provider 更新后老员工被旧版本号误判为失效；Windows 特殊路径直接交给系统定位且调用没有结果反馈，界面可能提示成功却不打开文件夹。当前修复让老员工每次任务使用同一 Provider 的当前有效配置，并改为打开经过转换的真实父文件夹、检查系统返回结果；完整工程检查、最终打包程序窗口和当前修复的 Windows/macOS CI 已通过，但仍需用户使用新安装包复验，所以不能标记完成。通过后也只代表可信成果展示完成，不代表标准 Skill、附件、文档、图片或多员工已经完成。
