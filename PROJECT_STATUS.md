...
| --- | --- |
| 当前产品版本 | Pi 重启版真实工作阶段 |
| 当前阶段 | Milestone 12 首个任务已形成验收候选版 |
| 当前 Milestone | Milestone 12：标准 Agent Skills 运行底座 |
| 当前任务单元 | M12-TU-01（进行中） |
| 总体状态 | Milestone 7–11 已完成；M12-TU-01 的本地工程、真实窗口和 Windows 打包检查已通过，17 项本地断言已有证据；同一提交的 Windows/macOS CI 和用户人工验收尚未完成，不得标记完成 |
| 最近更新 | 2026-08-23 |
| 下一检查点 | 提交并推送 M12-TU-01 验收候选版，等待同一提交的 Windows/macOS CI 通过，再交给用户安装并人工验收 |
...
M12-TU-01 已形成验收候选版：一名员工可选择多项 Skill，旧员工会保留原 Skill；任务开始时模型只看到名称和用途，再按任务启用完整说明、读取参考资料或复制资源成真实成果。脚本运行和环境自动安装仍属于 M12-TU-02，没有混入当前任务。
...
当前没有外部阻塞。M12-TU-01 只剩同一候选提交的 Windows/macOS CI 和用户安装包人工验收两道门禁，未通过前保持“进行中”。用户已确认 M12 路线：一名员工拥有多项 Skill，员工按任务自动选择；使用专门工具启用 Skill、列出/读取参考资料、复制资源和运行脚本；缺少环境时由独立环境管理工具提供自动安装，优先安装到项目或 Skill 独立环境，系统级安装必须再次确认。M12-TU-01 先交付多 Skill 和资源闭环，M12-TU-02 再交付脚本和环境；未来环境员工复用同一工具，真实公开 Skill 另做端到端验收。
...
- 当前尚无验收候选提交的 Windows/macOS GitHub Actions 证据，也尚未取得用户对该安装包的人工验收，因此 M12-TU-01 和 Milestone 12 均不得标记完成。
