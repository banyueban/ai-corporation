...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | Milestone 15 已确定共享工作区和单一文件负责人方案，仍有两项产品行为待确认                                                                                                                                                                               |
| 当前 Milestone | Milestone 15：轻量多员工协作（规划中）                                                                                                                                                                                                                 |
| 当前任务单元   | M15-TU-01（未开始，公司协作入口和失败/取消方式未确认）                                                                                                                                                                                                  |
| 总体状态       | Milestone 7–14 全部完成；M15 已拆为文档协作与单一编码员工协作，不做多人同改文件、worktree 或自动合并；首个合同尚未就绪，不得开始生产实现                                                                                                                 |
| 最近更新       | 2026-09-08                                                                                                                                                                                                                                              |
| 下一检查点     | 用户确认怎样发起公司协作并确定负责人，以及员工失败或整项取消后怎样继续；随后补齐 UI 与数据设计并把 M15-TU-01 标记为“就绪”                                                                                                                              |
...
Milestone 15 已确定轻量方案但尚未开始实现：多名员工共享同一个 Workspace，一个文件只有一名负责人；编码任务只由一名编码员工修改代码和运行项目命令，其他员工通过独立结果或检查意见协助；不创建 worktree、员工分支或自动合并。M15-TU-01 先验收文档协作，M15-TU-02 再验收编码协作，两项都通过后才能关闭 Milestone。公司协作入口和失败/取消方式仍待用户确认。正式版 UI 设计与重做已安排为 Milestone 15 后的独立方向 `DE-022`。
...
- 公司协作怎样发起并确定负责人，以及员工失败或整项任务取消后怎样继续，尚未确认；
- M15-TU-01 与 M15-TU-02 均未开始，没有任何多员工功能被标记为完成。

## 4. 当前任务边界

当前只允许完善 M15-TU-01 的规划和合同，不得在剩余两项行为确认前修改生产协议、Schema、迁移或实现。已经确定的边界是：

- M15-TU-01 只交付文档类协作，多名员工通过分工和交接共同完成任务，最终 Word/PDF 只有一名负责人修改和交付；
- 同一任务使用一个 Workspace，一个文件只允许明确负责人修改，不同员工可以分别负责不同文件；
- M15-TU-02 在 M15-TU-01 完成后单独交付编码协作，代码和项目命令始终只由一名编码员工负责；
- 分支、worktree、自动合并、多人同时修改同一文件和正式版 UI 都不属于 M15-TU-01。

当前规划更新不能被当成功能实现、开发态验收、最终包验收或 Milestone 完成。

## 5. 活跃阻塞与已确认决策

当前阻塞只有两项产品决定：第一，用户从哪里发起公司协作、由谁确定最终负责人和参与员工；第二，某名员工失败或用户取消整项任务时，已完成交接怎样保留、是否继续其他分工以及怎样恢复。两项会改变 UI、状态和数据，确认前 M15-TU-01 保持“未开始”。
...
当前下一步是由用户确认公司协作入口和失败/取消方式。确认后先更新产品、Agent Runtime、UI 流程、状态、协议、数据和 M15-TU-01 合同，运行文档门禁并将合同标记为“就绪”；然后才开始 M15-TU-01 文档协作实现。M15-TU-01 人工验收通过后再启动 M15-TU-02，Milestone 15 完成后进入正式版 UI 设计与重做。
