...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 当前产品版本   | Pi 重启版真实工作阶段                                                                                                                                                                                                                                   |
| 当前阶段       | M15-TU-02 首轮人工验收发现操作反馈和 Windows 命令问题，修复已通过本地与 Windows 成品自动验收，等待新候选 CI 和用户复验                                                                                                                                    |
| 当前 Milestone | Milestone 15：轻量多员工协作（实施中）                                                                                                                                                                                                                 |
| 当前任务单元   | M15-TU-02（进行中）                                                                                                                                                                                                                                       |
| 总体状态       | Milestone 7–14 和 M15-TU-01 全部完成；M15-TU-02 首轮真实任务最终成功，但暴露两个验收问题，当前修复已通过本地与 Windows 成品自动检查；新候选的双平台 CI 和用户复验尚未完成，Milestone 15 仍在实施中                                                        |
| 最近更新       | 2026-09-09                                                                                                                                                                                                                                                |
| 下一检查点     | 推送包含操作反馈和跨平台命令修复的新候选，等待 Windows/macOS CI，通过后由用户使用当前 Windows 安装包复验                                                                                                                                                 |
...
M14-TU-01 与 Milestone 14 已完成：用户可把 Word、PDF、文本或 Markdown 作为任务附件交给员工，软件保存任务副本并通过标准文档 Skill 读取内容，在 Workspace 生成新的 Word/PDF 成果；常用排版不再显示 Markdown 标记。长文档、成果核对和格式问题均已修复，本地完整检查、开发态 11 条、Windows 最终包 8 条、提交 `9a5a02c` 的 Windows/macOS CI 以及用户人工验收全部通过，P0/P1 为 0。

M15-TU-01 已完成：多名员工共享同一个 Workspace，负责人按 Skill 安排协助员工，协助员工只读资料并交接，最终 Word/PDF 只由一名负责人生成和修改；分工、过程、失败处理、停止、重启、退回修改和用户验收均已取得本地、最终程序、双平台 CI 与用户人工证据。M15-TU-02 将在同一底座上验收编码协作，代码和项目命令只由一名编码员工负责；它通过前 Milestone 15 不关闭。正式版 UI 设计与重做已安排为 Milestone 15 后的独立方向 `DE-022`。
...
- [x] 当前格式修复的本地完整检查和开发态真实窗口回归；
- [x] 当前格式修复的最终 Windows 打包程序验收；
- [x] 当前格式修复的 Windows/macOS CI；
- [x] 用户 Windows 安装包人工验收。

## 3H. Milestone 15 规划状态

- 已确认多名员工共享同一个任务 Workspace，一个文件只由一名负责人修改；
- 已确认编码任务只有一名编码员工修改代码和运行项目命令，其他员工只交接要求、资料或检查意见；
- 已确认不实现 Git worktree、员工分支、自动合并或普通文件通用合并；
- 已确认 M15-TU-01 验收文档协作，M15-TU-02 验收单一编码员工的编码协作，两项都是 Milestone 15 必需范围；
- 多人同时修改同一文件与隔离工作区已登记为 `DE-023`，只有真实使用反复需要时再评估；
- 已确认从公司工作区发起协作，用户选择最终负责人和 Workspace，负责人按 Skill 自行安排当前公司协助员工，分工立即执行且不二次确认；
- 已确认协助员工失败不自动重试，负责人能继续就继续、不能继续就询问用户；取消停止所有仍在运行的员工，保留未完成成果且不自动恢复；
- M15-TU-01 的 12 项验收断言全部通过；M15-TU-02 已按 `1A + 2A + 3A` 实现，首轮人工真实任务最终成功但暴露操作反馈和 Windows 命令问题，修复已通过本地与 Windows 成品自动验收；新候选双平台 CI 和用户复验尚未完成，Milestone 15 不标记完成。

## 4. 当前任务边界

M15-TU-01 已关闭，M15-TU-02 正在按已就绪合同验收。已冻结边界是：

- M15-TU-01 只交付文档类协作，多名员工通过分工和交接共同完成任务，最终 Word/PDF 只有一名负责人修改和交付；
- 同一任务使用一个 Workspace；协助员工只读资料并交回文字，只有最终负责人可以生成或修改最终文件；
- 公司协作使用独立入口，不改变现有单员工任务；负责人按 Skill 选协助员工，失败不自动重试，无法继续时由用户决定；
- M15-TU-02 在 M15-TU-01 完成后单独交付编码协作，代码和项目命令始终只由一名编码员工负责；
- 分支、worktree、自动合并、多人同时修改同一文件和正式版 UI 都不属于 M15-TU-01。

M15-TU-01 完成不能被当作 M15-TU-02 已实现、已验收或 Milestone 15 已完成。

## 5. 活跃阻塞与已确认决策

当前没有活跃产品阻塞。M15-TU-02 按用户选择的 `1A + 2A + 3A` 实施：最终负责人就是唯一编码员工；复用“开始公司协作”；日常任务不强制组队，但连续验收必须使用至少一名协助员工。它不重新引入任务类型、组队确认、员工分支或自动合并。

用户已确认的 M15 决策是：复杂任务才使用多名员工；共享一个 Workspace；一个文件一个负责人；代码只由一名编码员工修改；其他员工以独立结果和意见交接；最终负责人汇总、自查并交给用户验收；文档与编码分成两个任务单元；不做 worktree、分支和自动合并。正式版 UI 已安排在 Milestone 15 之后，登记为 `DE-022`；同文件并行编辑与隔离工作区登记为按需评估的 `DE-023`。
...
- M14-TU-01 常用格式修复的本地 `pnpm check` 通过：协议 71、Provider 28、存储 110、桌面 208、Native Core 11、Workspace FS 12，并通过状态、任务合同、格式、类型、Clippy 和 Secret scan；
- 开发态真实窗口 11 条通过、5 条按旧入口或真实 Key 开关跳过；文档旅程真实生成并重新打开带粗体、斜体、删除线、行内代码、引用、分隔线、安全链接、两级列表和带行内格式表格的 Word/PDF，验证对应 Markdown 标记不再残留；
- Windows 最终打包程序 8 条真实窗口旅程全部通过；提交 `9a5a02cfe70a9bcc341fe1bb213fe5a9b62deb17` 的 GitHub Actions run `34130814174` 已通过，Windows x64 用时 9 分 52 秒、macOS Apple Silicon 用时 6 分 54 秒，两边均通过开发态 11 条和最终包 8 条旅程并上传安装包；
- 当前本地验收包位于 `release/M14-9a5a02c/AI Corporation Desktop Setup 0.1.0.exe`，大小 152753697 字节，SHA-256 为 `4E089F68B1C36A23DEBD69AECFD7F6EA6447F66F1AE29600C6BB930799F8CA8F`；GitHub CLI 下载远端 Windows artifact 两次都长时间无字节落盘，因此没有把未完成下载冒充 CI 本地副本。该本地包来自同一源码并已通过本机最终包 8 条旅程。
- 用户于 2026-09-07 使用当前 Windows 安装包完成真实 Word/PDF 常用格式人工验收并确认通过；M14-TU-01 的 21 项断言全部关闭，P0/P1 为 0，M14-TU-01 与 Milestone 14 完成。
- M15-TU-01 已新增普通/公司协作任务区分、持久化分工与员工过程、负责人委派与等待用户工具、协助员工只读工具边界、停止和退回恢复、公司协作入口、分工交接展示以及 Windows 真实窗口旅程；
- 本地 `pnpm check` 通过：协议 72、Provider 28、存储 111、桌面 210、Native Core 11、Workspace FS 12，并通过格式、类型、Clippy 和 Secret scan；
- 开发态完整窗口 12 条通过、5 条按旧入口或真实 Key 开关跳过；M15 专项真实执行负责人调用协助员工、文字交接、负责人生成并核对 Word、等待用户决定、使用已有结果继续、退回修改、停止任务和应用重启不重放，1024×700、1440×900 与 200% 缩放无横向溢出；
- Windows `win-unpacked` 最终程序已通过同一 M15 连续窗口旅程及停止后重启检查；安装包位于 `release/AI Corporation Desktop Setup 0.1.0.exe`，大小 152753194 字节，SHA-256 为 `0C72DA38FB1EE65B6C575E82B6061B00560A68241101812DE09977A5E9045301`；
- 验收候选提交 `6484b5f4ba6203a264fc27a344d5626e0623d4f4` 的 GitHub Actions run `34176910807` 已通过：macOS Apple Silicon 7 分 24 秒、Windows x64 8 分 12 秒，两边均完成工程检查、真实窗口、应用构建、最终打包程序窗口检查和安装包上传；只有 GitHub Action 的 Node.js 20 弃用提示，没有产品失败；
- 用户于 2026-09-08 使用当前 Windows 安装包完成人工验收并确认通过；M15-TU-01 的 12 项断言全部关闭，P0/P1 为 0。该结论只关闭文档协作，不代替 M15-TU-02 或 Milestone 15。
- M15-TU-02 已实现唯一编码员工规则：最终负责人拥有代码写入、成果登记、脚本和项目命令工具，协助员工只拥有读取与文字交接能力；同一编码员工根据检查意见修订并重新运行检查，不创建 `.git`、员工分支或 worktree；
- M15-TU-02 专项单元测试 8 条通过；本地 `pnpm check` 通过：协议 72、Provider 28、存储 111、桌面 210、Native Core 11、Workspace FS 12，并通过状态、任务合同、格式、类型、Clippy 和 Secret scan；
- 开发态完整窗口回归 13 条通过、5 条按旧入口或真实 Key 开关跳过；M15-TU-02 连续旅程真实完成两次协助交接、唯一编码员工写入 `sort.js`、两次项目检查、检查意见修订、退回修改和第三次检查，且在 1024×700、1440×900 与 200% 缩放下无横向溢出；
- Windows `win-unpacked` 最终程序已通过同一 M15-TU-02 连续编码协作旅程。安装包位于 `release/AI Corporation Desktop Setup 0.1.0.exe`，大小 152753044 字节，SHA-256 为 `67B13B40907C9CFB58F5F31F9DE29BEC0B24968E591CF43629265B7D7AF8CA82`；同一候选提交的双平台 CI 和用户人工验收仍待完成，不能标记 M15-TU-02 或 Milestone 15 完成。
- 原候选提交 `ee18a65` 的 GitHub Actions run `34305924499` 已通过：macOS Apple Silicon 6 分 47 秒、Windows x64 9 分 20 秒；但随后用户首轮人工验收发现所有等待型操作没有明确的点击中、成功或失败反馈，并且 Windows 任务中模型先后两次使用 `rm -f` 导致命令失败，因此该候选不能作为最终通过证据；
- 首轮人工任务 `01a0846a-f683-78f9-b518-f5cb2ebcd0a6` 最终处于“等待验收”：协助员工两次只读交接均成功，唯一编码员工完成 `sort.js` 和 `test-sort.js`，本机重新运行 22 项测试全部通过；中途失败与最终成功均如实保留，没有把失败过程隐藏或把任务提前标记完成；
...
当前下一步是推送包含操作反馈和跨平台命令修复的 M15-TU-02 新候选，等待 Windows/macOS CI 后由用户使用当前 Windows 安装包复验。两项都通过后才能关闭 M15-TU-02 和 Milestone 15，之后进入正式版 UI 设计与重做。
...
Milestone 14 的目标与实际交付一致：用户可以把真实 Word、PDF、文本和 Markdown 直接交给员工，员工读取固定副本并生成新的 Word/PDF 成果。人工验收先后发现长文档生成失败、模型误读成果文件名以及 Markdown 标记残留，均在关闭前修复并重新走完本地、最终包、双平台 CI 和用户验收。文档存在和进程成功不能代替真实内容及排版核对，这条门禁继续沿用。
