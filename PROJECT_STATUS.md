...
| --- | --- |
| 当前产品版本 | Pi 重启版真实工作阶段 |
| 当前阶段 | Milestone 12 第二个任务等待人工复验技能导入 |
| 当前 Milestone | Milestone 12：标准 Agent Skills 运行底座 |
| 当前任务单元 | M12-TU-02（进行中） |
| 总体状态 | Milestone 7–11 和 M12-TU-01 已完成；M12-TU-02 首次人工验收在导入验收 Skill 时失败，原因是提供的本地文件夹名称与 SKILL.md 的 name 不一致，且软件没有把原因显示在操作位置；当前修复候选已通过本地完整工程、开发态真实窗口、Windows 最终打包程序和同一提交的 Windows/macOS CI，等待用户先复验技能导入；Milestone 12 尚未验收完成 |
| 最近更新 | 2026-08-25 |
| 下一检查点 | 用户使用当前 Windows 安装包先复验“导入技能文件夹”一步；首步通过后再继续环境计划、自动安装、脚本过程和真实成果验收 |
...
- [ ] M12-TU-02 用户人工验收；首次验收在导入验收 Skill 时失败，当前修复候选的 Windows 本地开发态、最终打包程序和同一提交的 Windows/macOS CI 已通过；
...
当前没有代码或外部阻塞。M12-TU-02 当前修复候选已经取得同一提交的 Windows/macOS CI，下一步是用户先重新验收 Skill 导入。首次人工验收已经明确失败，不能沿用之前的人工验收结论。用户已确认：一名员工拥有多项 Skill 并按任务自动选择；JavaScript 使用软件运行能力，Python 缺失时装入软件自管目录，Windows/macOS 分别支持 PowerShell/Shell；自动识别 PEP 723、requirements、package.json 和结构化包名/版本；默认使用 Skill 独立环境；系统程序只用受限 winget/Homebrew 方案并再次确认。未来环境员工复用同一工具，真实公开 Skill 另做端到端验收。
